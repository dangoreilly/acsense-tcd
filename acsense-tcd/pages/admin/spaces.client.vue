	...
	export default {
		data() {
			return {
				...
				space_list: [],
				space_list_loaded: false,
				space_types: [],
				space_types_loaded: false,
				buildings: [], 
		...
		watch: {
			buildings: {
				handler() {
					// Whenever the buildings list loads, check if the map has loaded
					if (this.mapDataLoaded()) {
						// Update the map with the buildings
						this.loadSpaceToMap();
					}
				},
				deep: true
		...
		methods: {
			...
			mapDataLoaded(){
				...
				// Checks if the map data has loaded
				if (!this.space_types_loaded) {
					console.log("Space types not loaded yet")
					return false;
				} 
				if (!this.buildings_loaded) {
					console.log("Buildings not loaded yet")
					return false;
				} 
				if(!this.space_list_loaded) {
					console.log("Space list not loaded yet")
					return false;
				} 
				if(!this.overlays_loaded) {
					console.log("Overlays not loaded yet")
					return false;
				} 
				
				// Check that the map element is present
				if(document.getElementById("space-placement-map") == null) {
					console.log("Space placement map not loaded yet")
					return false;
				} 
